...
\section{Rolle der Logik in MAS}
	...
	\begin{itemize}
		...
		\item Regeln:
		\begin{itemize}
			\item (Des $\alpha$) $\rightarrow$ (Bel $\alpha$) (Belief goal compatibility)
			\item (Int $\alpha$) $\rightarrow$ (Des $\alpha$) (Goal-intention compatibility)
			\item (Int does(a)) $\rightarrow$ does(a) (Volitional commitment)
			\item (Des $\alpha$) $\rightarrow$ (Bel (Des $\alpha$)) und (Int $\alpha$) $\rightarrow$ (Bel (Int $\alpha$)) (Awareness of goals \& intentions)
			\item done(a) $\rightarrow$ (Bel (done(a))) (No unconscious actions)
			\item (Int $\alpha$) $\rightarrow$ AF($\neg$(Int $\alpha$)) (No infinite deferral)
		\end{itemize}
		\item TODO: ????
	\end{itemize}
